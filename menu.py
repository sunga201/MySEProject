...
class ShowRobotMovement(WindowDesign):
    def __init__(self):
        ...
        self.drawInitialMap() # 창에 넣을 맵을 matplotlib을 이용해 만들고 로봇, 초기 경로, hazard spot, object spot을 그린다. 만든 맵은 canvas에 담아 PyQt5로 만든 창에 넣는다.
        self.designShowRobotMovementScreen()  # 창의 디자인을 담당하는 메소드 호출

    '''
       setInitialMapInfo()에서 선언된 클래스 변수 설명
            arrowDirection : 방향을 나타내는 화살표가 로봇에서 얼만큼 떨어져 있는지를 결정한다. 차례대로 위, 오른쪽, 아래, 왼쪽
            arrowFileName : 화살표 이미지 파일 경로 저장
            curDirection : 현재 로봇의 방향을 저장한다.
            mapSize : 맵의 크기 저장
            curPosition : 현재 로봇의 위치를 저장한다.
            hazardSpot : 오퍼레이터로부터 주어진 hazard spot 좌표들을 저장한다.
            originalObjectSpot : 오퍼레이터로부터 주어진 object spot 좌표들을 저장한다. 동작이 끝난 뒤 재실행할 때 MapData 클래스에 저장된 object spot 리스트를 복구할 떄 사용한다.
            objectSpot : 오퍼레이터로부터 주어진 object spot 좌표들을 저장한다. 로봇이 특정 object spot을 방문하면 MapData 클래스에 저장되어 있는 object spot 리스트에서 해당 지점이 제거된다.
            objCnt : 아직 방문하지 않은 object spot 갯수를 저장한다.
    ...
    def drawInitialMapIcon(self):
        ...
        for hiddenHSpot in self.hiddenHazardSpot: # 무작위로 생성된 hidden hazard spot을 맵에 흐리게 표시한다.
            self.hiddenHazardSpotInstance.append((hiddenHSpot, self.drawImage(hiddenHSpot[0], hiddenHSpot[1], 'image/skull_dim.png')))

        for hiddenCBSpot in self.hiddenColorBlobSpot: # 무작위로 생성된 color blob spot을 맵에 흐리게 표시한다.
            self.hiddenColorBlobSpotInstance.append((hiddenCBSpot, self.drawImage(hiddenCBSpot[0], hiddenCBSpot[1], 'image/splash_dim.png')))

    def designShowRobotMovementScreen(self):
        # 디자인 코드 시작
        ...
        self.addWidgetOnLayout(leftLayout, self.canvas)
    ...
    '''
        drawInitialMap() : matplotlib을 이용해 맵을 만들고 로봇, 초기 경로, hazard spot, object spot을 그린다. 만든 맵은 canvas에 담아 PyQt5로 만든 창에 넣는다.
          - 선언된 클래스 변수 -
            ...
            enlarge : 맵의 가로, 세로 길이를 참조하여 맵의 확대 여부를 결정한다.
    '''
    def drawInitialMap(self):
        ...
        self.mapScreen.grid()
        self.hiddenHazardSpot = map_data.MapData.getHazardH()
        self.hiddenColorBlobSpot = map_data.MapData.getCbH()
        self.drawRobot() # 맵 상에 로봇을 그린다.
        self.drawInitialMapIcon()
        self.drawPath() # MapData 클래스에 저장된 경로 정보를 바탕으로 맵 상에 경로를 그린다.
        ...
        while self.objCnt!=0:
            ...
            if len(hiddenCbList)!=0: #현재 위치 근처에 hidden cb가 존재한다.
                for pos in hiddenCbList: #리스트에는 color blob의 좌표가 들어있다.
                    self.eraseAndDrawImage(pos, self.hiddenColorBlobSpotInstance, 'image/splash.png')
            ...
            if self.ctrlRobot.checkDirection() and hiddenHSpot[0]!=-1:
                ...
                self.eraseAndDrawImage(hiddenHSpot, self.hiddenHazardSpotInstance, 'image/skull.png')
                self.changePath()
                hazardFound=True
            ...
            if tuple(self.curPosition) in self.objectSpot:
                self.eraseAndDrawImage(self.curPosition, self.objectSpotInstance, 'image/star_red.png')
    ...
    def commandMovementAndChangePosInfo(self, hazardFound):
        ...
        self.drawRobot(changePath=isChangePath)
    ...
    def drawRobot(self, changePath=False): # changePath
        ...
        self.robotImage = self.drawImage(self.curPosition[0], self.curPosition[1], 'image/robot.png')
        print(type(self.robotImage))
        self.arrowImage = self.drawImage(self.curPosition[0] + arrowDirection[self.curDirection][0],
                                            self.curPosition[1] + arrowDirection[self.curDirection][1], arrowFileName[self.curDirection])
        ...
        if self.enlarge:# 맵 가로, 세로 길이 둘 다 10 초과
            ...
            plt.ylim(-3 + self.curPosition[1], 4 + self.curPosition[1])
            plt.yticks(np.arange(-3 + self.curPosition[1], 4 + self.curPosition[1], step=1))
        if not changePath or self.mapSize[0]+self.mapSize[1]<300 : time.sleep(self.delay) # delay초 간격으로 로봇의 움직임 표현.
