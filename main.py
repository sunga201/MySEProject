...
main_menu=uic.loadUiType('main menu.ui')[0]
save_data=uic.loadUiType('save data.ui')[0]
print(main_menu)
...
class SaveData(QMainWindow, save_data):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
    ...
    def saveInputData(self):
        mapSize = self.mapText.text()
        start = self.startText.text()
        spot = self.objText.text()
        hazard = self.hazardText.text()
...
class ShowMapData(QWidget):
    def __init__(self):
        ...
        mapSizeText.setStyleSheet(
            'background:rgb(176, 191, 197)')  # qLineEdit 색 변경. 참조 사이트 : http://blog.naver.com/PostView.nhn?blogId=bringblingme&logNo=221544905595&redirect=Dlog&widgetTypeCall=true&directAccess=false
...
class ShowResult(QWidget):
    ...
    def showMap(self):
        ...
        initialDirection = self.path[1][0] - self.path[0][0], self.path[1][1] - self.path[0][
        ...
        for objSpot in self.objectSpot:
            self.objectSpotInstance.append(
                (objSpot, self.imageScatter(objSpot[0], objSpot[1], 'star.png', zoom=0.1, ax=self.mapScreen)))
    ...
    def commandMovementAndChangePosInfo(self):
        ...
        if changedDirection == self.curDirection and abs(
                beforePosition[0] - changedPosition[0] + beforePosition[1] - changedPosition[1]) == 0:
        ...
        self.drawRobot(self.curPosition)
        print('before : ', beforePosition, ', cur : ', self.curPosition, 'dis : ',
              abs(beforePosition[0] - self.curPosition[0] + beforePosition[1] - self.curPosition[1]))
    ...
    def drawRobot(self, test):
        ...
        self.arrowImage = self.imageScatter(self.curPosition[0] + x + self.arrowDirection[self.curDirection][0],
                                            self.curPosition[1] + y + self.arrowDirection[self.curDirection][1],
                                            self.arrowFileName[self.curDirection],
    ...
    def generateRandomSpot(self):
        ...
        hazardNum = np.random.randint(0, min((self.mapSize[0] + self.mapSize[1]) / 2,
                                             100))  # 보이지 않는 위험 지역의 수를 0 ~ (맵의 가로 길이 + 세로 길이) (최대 100) 사이의 수로 정한다.
        cbNum = np.random.randint(0, min((self.mapSize[0] + self.mapSize[1]) / 2,
                                         100))  # color blob spot의 수를 0 ~ (맵의 가로 길이 + 세로 길이) (최대 100) 사이의 수로 정한다.
        ...
        for i in range(len(self.path) - 1):
            ...
            if (x == 0):
                if (self.path[i][1] < self.path[i + 1][1]):
                    lines = self.mapScreen.plot([self.path[i][0]] * (y + 1),
                                                np.linspace(self.path[i][1], self.path[i + 1][1], y + 1), 'r',
                                                linewidth=5)
                else:
                    lines = self.mapScreen.plot([self.path[i][0]] * (-(y - 1)),
                                                np.linspace(self.path[i + 1][1], self.path[i][1], -(y - 1)), 'r',
                                                linewidth=5)
            else:
                if (self.path[i][0] < self.path[i + 1][0]):
                    lines = self.mapScreen.plot(np.linspace(self.path[i][0], self.path[i + 1][0], x + 1),
                                                [self.path[i][1]] * (x + 1), 'r',
                                                linewidth=5)

                else:
                    lines = self.mapScreen.plot(np.linspace(self.path[i + 1][0], self.path[i][0], -(x - 1)),
                                                [self.path[i][1]] * -(x - 1), 'r',
                                                linewidth=5)
...
class ShowMenu(QMainWindow, main_menu):
    def __init__(self):
        super().__init__()
        ...
        self.showDataButton.clicked.connect(self.showMapData)
